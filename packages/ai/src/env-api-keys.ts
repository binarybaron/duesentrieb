...
function getApiKeyEnvVars(provider: string): readonly string[] | undefined {
	...
	// ANTHROPIC_AUTH_TOKEN and ANTHROPIC_OAUTH_TOKEN participate in env
	// discovery/status, but getEnvApiKey() skips them because request paths must
	// pass them as Authorization: Bearer.
...
export function getEnvApiKey(provider: string, env?: ProviderEnv): string | undefined {
	...
	if (envKeys?.[0]) {
		const apiKeyEnv =
			provider === "anthropic"
				? envKeys.find((key) => key !== ANTHROPIC_AUTH_TOKEN_ENV && key !== ANTHROPIC_OAUTH_TOKEN_ENV)
				: envKeys[0];
