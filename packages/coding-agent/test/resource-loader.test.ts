...
Project skill`,
		...
		it("should prefer .pi skills over .claude skills over .agents skills", async () => {
			const skillLocations = [
				{ dir: join(cwd, ".pi", "skills", "collision-skill"), description: "pi" },
				{ dir: join(cwd, ".claude", "skills", "collision-skill"), description: "claude" },
				{ dir: join(cwd, ".agents", "skills", "collision-skill"), description: "agents" },
			];
			for (const location of skillLocations) {
				mkdirSync(location.dir, { recursive: true });
				writeFileSync(
					join(location.dir, "SKILL.md"),
			...
			const loader = new DefaultResourceLoader({ cwd, agentDir });
			await loader.reload();
			expect(loader.getSkills().skills.find((skill) => skill.name === "collision-skill")?.description).toBe("pi");

			rmSync(join(cwd, ".pi", "skills"), { recursive: true, force: true });
			await loader.reload();
			expect(loader.getSkills().skills.find((skill) => skill.name === "collision-skill")?.description).toBe(
				"claude",
			);
		});
...
Content`,
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			...
			const claudeSkillDir = join(cwd, ".claude", "skills", "claude-project-skill");
...
Project skill content`,
			...
			expect(loader.getSkills().skills.some((skill) => skill.name === "claude-project-skill")).toBe(false);
...
Extra content`,
	...
	describe("noSkills option", () => {
		it("should skip skill discovery when noSkills is true", async () => {
			const skillsDir = join(agentDir, "skills");
			const claudeSkillDir = join(cwd, ".claude", "skills", "claude-skill");
			mkdirSync(skillsDir, { recursive: true });
			mkdirSync(claudeSkillDir, { recursive: true });
			writeFileSync(
				join(skillsDir, "test-skill.md"),
